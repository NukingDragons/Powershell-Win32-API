...
class STARTUPINFOW : BaseWin32Class
{
	[UInt32] $cb = $this.Size()
	[String] $lpReserved = ""
	[String] $lpDesktop = ""
	...
	[IntPtr] ToUnmanaged()
	{
		...
		[IntPtr] $lpReservedUni = [IntPtr]::Zero
		[IntPtr] $lpDesktopUni = [IntPtr]::Zero
		[IntPtr] $lpTitleUni = [IntPtr]::Zero

		if ($this.lpReserved.Length -gt 0)
		{
			$lpReservedUni = [System.Runtime.InteropServices.Marshal]::StringToHGlobalUni($this.lpReserved)
		}

		if ($this.lpDesktop.Length -gt 0)
		{
			$lpDesktopUni = [System.Runtime.InteropServices.Marshal]::StringToHGlobalUni($this.lpDesktop)
		}

		if ($this.lpTitle.Length -gt 0)
		{
			$lpTitleUni = [System.Runtime.InteropServices.Marshal]::StringToHGlobalUni($this.lpTitle)
		}
		...
		[IntPtr[]] $Strings = @($lpReservedUni, $lpDesktopUni, $lpTitleUni)
	...
	[STARTUPINFOW] FromUnmanaged([IntPtr] $Unmanaged)
	{
		...
		{
			$this.lpReserved = [System.Runtime.InteropServices.Marshal]::PtrToStringUni($Strings[0])
		}

		if ($Strings[1] -ne [IntPtr]::Zero)
		{
			$this.lpDesktop = [System.Runtime.InteropServices.Marshal]::PtrToStringUni($Strings[1])
		}

		if ($Strings[2] -ne [IntPtr]::Zero)
		{
			$this.lpTitle = [System.Runtime.InteropServices.Marshal]::PtrToStringUni($Strings[2])
		}
